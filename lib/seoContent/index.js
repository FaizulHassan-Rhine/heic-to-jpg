...
import { aiToolsSeo } from "./aiTools";
import { PRIVACY_CLIENT_SIDE } from "./shared";
import { getSampleSitemapPaths, getFormatBySlug } from "../sampleFiles";
...
export const LANDING_PAGE_SEO = {
  "heic-to-jpg": {
    id: "heic-to-jpg",
    path: "/heic-to-jpg",
    toolPath: "/convert?from=heic&to=jpg",
...
export const STATIC_PAGE_SEO = {
  ...
  "/about": {
    ...
    description:
      "Learn about ConvertMastery: free AI paraphraser, summarizer, email writer, image upscaler, plus privacy-first file conversion and PDF tools.",
  ...
  "/guide": {
    path: "/guide",
    title: "ConvertMastery Guide – How to Use All Tools Including AI",
    description:
      "Complete guide to ConvertMastery: AI paraphraser, summarizer, email writer, image upscaler, file converters, PDF tools, and privacy utilities.",
  ...
  "/blog": {
    ...
    description:
      "Tips and guides for file conversion, AI paraphrasing, PDF summarization, image compression, and online privacy from ConvertMastery.",
    h1: "Blog & Guides",
  },
  "/sample-files": {
    path: "/sample-files",
    ...
    description:
      "Download free sample JPG, PNG, WebP, AVIF, and HEIC images to test image converters, compressors, and AI upscalers. No signup required.",
    h1: "Sample Image Files",
  },
};
...
export function getSeoByPath(pathname) {
  ...
  const sampleMatch = normalized.match(/^\/sample-files\/([a-z0-9-]+)$/i);
  if (sampleMatch) {
    const format = getFormatBySlug(sampleMatch[1]);
    if (format) {
      return {
        type: "landing",
        path: `/sample-files/${format.slug}`,
        title: `Free Sample ${format.label} Files – Download & Test | ConvertMastery`,
        description: `Download free sample ${format.label} (${format.ext}) images to test converters, compressors, and AI upscalers. No signup required.`,
        h1: `Sample ${format.label} Files`,
      };
    }
  }
...
export function getAllSitemapPaths() {
  ...
  return [
    ...new Set([...staticPaths, ...toolPaths, ...landingPaths, ...getSampleSitemapPaths()]),
  ];
}
