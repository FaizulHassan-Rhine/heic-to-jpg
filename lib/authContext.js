...
export const AuthContextProvider = ({ children }) => {
  ...
  // Track tool usage for both logged-in and anonymous users
  const trackUsage = async (toolPath, count = 1, fileCount = 1, metadata = {}, files = []) => {
    try {
      ...
      console.log(`trackUsage: Sending ${(bodyString.length / 1024).toFixed(1)}KB to /api/auth/track-usage (${files.length} files, user: ${user ? 'logged-in' : 'anonymous'})`);
      ...
      if (!response.ok) {
        console.error("trackUsage API error:", response.status, result);
      } else {
        console.log("trackUsage success:", result);
      }
    } catch (error) {
      console.error("trackUsage fetch error:", error.message);
    }
  };
