    ...
    <ToolPageShell containerClassName="max-w-6xl">
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-8 lg:gap-10">
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start">
            <div className="rounded-xl border border-border dark:border-border bg-card dark:bg-card/50 p-5 shadow-sm">
              <h1 className="text-xl font-bold text-foreground dark:text-foreground mb-1">
      ...
      <CardContent className="space-y-4">
        ...
        {result && (
          <div className="p-4 rounded-lg bg-primary/10 text-primary font-medium">
            ...
            <br />
            <span className="text-sm text-muted-foreground dark:text-muted-foreground">Total: {result.totalDays} days</span>
          </div>
        )}
      </CardContent>
    </Card>
      ...
      <CardContent className="space-y-4">
        <div className="flex gap-2">
          <button
            ...
            className={cn("px-3 py-1.5 rounded text-sm font-medium", mode === "diff" ? "bg-primary text-primary-foreground" : "bg-muted dark:bg-muted")}
          >
            Date difference
          </button>
          <button
            onClick={() => setMode("add")}
            className={cn("px-3 py-1.5 rounded text-sm font-medium", mode === "add" ? "bg-primary text-primary-foreground" : "bg-muted dark:bg-muted")}
          >
            Add / Subtract days
          </button>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        {result != null && !loading && (
          <div className="p-4 rounded-lg bg-primary/10 text-primary font-medium">
            ...
            {rates?.date && <span className="block text-xs text-muted-foreground mt-1">Rate date: {rates.date}</span>}
          </div>
        )}
      </CardContent>
    </Card>
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        <div className="flex flex-wrap gap-2">
          <button onClick={() => setMode("percent")} className={cn("px-3 py-1.5 rounded text-sm font-medium", mode === "percent" ? "bg-primary text-primary-foreground" : "bg-muted dark:bg-muted")}>Percentage</button>
          <button onClick={() => setMode("discount")} className={cn("px-3 py-1.5 rounded text-sm font-medium", mode === "discount" ? "bg-primary text-primary-foreground" : "bg-muted dark:bg-muted")}>Discount</button>
          <button onClick={() => setMode("tip")} className={cn("px-3 py-1.5 rounded text-sm font-medium", mode === "tip" ? "bg-primary text-primary-foreground" : "bg-muted dark:bg-muted")}>Tip</button>
        </div>
        {mode === "percent" && (
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
